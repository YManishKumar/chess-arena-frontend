...
export class DashboardComponent implements OnInit {
  ...
  toggleChessCard() {
    this.showChessCard = !this.showChessCard;
  }

  shareCard() {
    const username = this.name || this.email.split('@')[0];
    const text = `Chess Card for ${username}\nRank: ${this.rank}\nPoints: ${this.points}\nStats: ${this.stats[0].value} games, ${this.stats[1].value} wins\nPlay me on Chess Arena! ♟️`;
  ...
  loadPoll() {
    ...
    this.api.get<Poll>(`/poll/today?user_id=${encodeURIComponent(this.email)}`).subscribe({
      next: p => { this.poll = p; this.userVote = p.user_voted ?? null; this.pollLoading = false; },
      error: () => { this.pollLoading = false; }
    });
  }

  votePoll(index: number) {
    if (this.pollVoting || this.userVote !== null) return;
    this.pollVoting = true;
    this.api.post<Poll>('/poll/vote', { user_id: this.email, option_index: index }).subscribe({
      next: p => { this.poll = p; this.userVote = index; this.pollVoting = false; },
      error: () => { this.pollVoting = false; }
    });
  }

  get maxPollPct(): number {
    return Math.max(...(this.poll?.options?.map(o => o.percentage) ?? [0]));
  }
