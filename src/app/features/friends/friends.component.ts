...
interface RivalryStats {
  ...
  draws: number;
  total_games: number;
  status: string; // "you're winning" | "they're winning" | "tied"
}
...
export class FriendsComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  // Rivalry / Beef
  rivalryStats: Record<string, RivalryStats | null> = {};
  rivalryLoading: Record<string, boolean> = {};

  // Emoji-only chat
  emojiOnlyMode = false;
  readonly speedEmojis = ['♟️', '♔', '♕', '⚔️', '💀', '🔥', '👑', '🤝', '😤', '🫡', '⚡', '🏆'];

  toggleEmojiMode() { this.emojiOnlyMode = !this.emojiOnlyMode; }

  sendEmojiMessage(emoji: string) {
    if (!this.selectedEmail || this.chatSending) return;
  ...
  loadFriends() {
    ...
    this.friendsService.getFriends(this.currentEmail).subscribe({
      next: res => {
        ...
        this.accepted.forEach(f => this.loadRivalry(this.friendEmail(f)));
      },
      error: () => { this.pageLoading = false; }
    });
  }

  loadRivalry(friendEmail: string) {
    if (this.rivalryStats[friendEmail] !== undefined) return;
    this.rivalryLoading[friendEmail] = true;
    this.api.get<RivalryStats>(`/friends/beef/${encodeURIComponent(this.currentEmail)}/${encodeURIComponent(friendEmail)}`).subscribe({
      next: stats => {
        this.rivalryStats[friendEmail] = stats;
        this.rivalryLoading[friendEmail] = false;
      },
      error: () => {
        this.rivalryStats[friendEmail] = null;
        this.rivalryLoading[friendEmail] = false;
      }
    });
  }
