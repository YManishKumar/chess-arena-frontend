...
export class ChessBoard3DComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  private hoverRingGeo!: THREE.TorusGeometry;

  // Hover state
  private hoveredSq: Square | null = null;
  private hoverRingMesh!: THREE.Mesh;
  ...
  private initThree() {
    ...
    this.renderer.toneMappingExposure = this.overhead ? 0.85 : 1.2;
    const bg = this.overhead ? 0x0b0f14 : 0x0a0820;
    ...
    // PMREMGenerator — studio env for subtle IBL reflections (NOT scene background)
    if (this.overhead) {
    ...
    this.camera = new THREE.PerspectiveCamera(this.overhead ? 45 : 50, W / H, 0.1, 1000);
    ...
    if (this.overhead) {
      // 3-point studio lighting matching spec
      // Low ambient — let key/fill do the work
      this.scene.add(new THREE.AmbientLight(0xfff8f0, 0.55));

      // Key light: warm, top-left as per spec diagram
      const key = new THREE.DirectionalLight(0xfff5e0, 5.5);
      key.position.set(-8, 16, 10); key.castShadow = true;
      key.shadow.mapSize.set(4096, 4096);
      Object.assign(key.shadow.camera, { left: -16, right: 16, top: 16, bottom: -16, near: 0.5, far: 80 });
      key.shadow.bias = -0.0004;
      key.shadow.normalBias = 0.015;
      this.scene.add(key);

      // Fill: soft warm-white, top-right
      const fill = new THREE.DirectionalLight(0xfff8f0, 2.2);
      fill.position.set(10, 14, 8); this.scene.add(fill);

      // Rim: warm from behind
      const rim = new THREE.DirectionalLight(0xffe8c0, 1.2);
      rim.position.set(0, 10, -18); this.scene.add(rim);

      // Gold point: low-level frame glow
      const glow = new THREE.PointLight(0xd4af37, 2.5, 20);
      glow.position.set(0, 1, 8); this.scene.add(glow);
      const glow2 = new THREE.PointLight(0xd4af37, 1.8, 20);
      glow2.position.set(0, 1, -8); this.scene.add(glow2);
    } else {
      // Cinematic lighting
    ...
    // Spec colors: light #F6F6F2, dark #111111, gold #D4AF37
    const lightTileColor = this.overhead ? 0xf6f6f2 : 0xe8d9b8;
    const darkTileColor  = this.overhead ? 0x111111 : 0x5c3218;
    const edgeColor      = this.overhead ? 0xd4af37 : 0x9070ff;
    const edgeEmissive   = this.overhead ? 0xc09020 : 0x9070ff;

    const lt = m({ color: lightTileColor, roughness: this.overhead ? 0.10 : 0.75, metalness: this.overhead ? 0.0 : 0.04 });
    const dt = m({ color: darkTileColor,  roughness: this.overhead ? 0.06 : 0.62, metalness: this.overhead ? 0.0 : 0.06 });
    if (this.overhead && this.envMap) {
      lt.envMap = this.envMap; lt.envMapIntensity = 0.55;
      dt.envMap = this.envMap; dt.envMapIntensity = 0.80;
    }
    ...
    this.M = {
      ...
      blackPc:   null as any,
      // Highlights
      selected:  m({ color: 0xf7c230, emissive: 0xf7c230, emissiveInt: 0.70, transparent: true, opacity: 0.88, roughness: 0.2, metalness: 0.15 }),
      legal:     m({ color: 0xd4a017, emissive: 0xd4a017, emissiveInt: 0.55, transparent: true, opacity: 0.72, roughness: 0.3, metalness: 0.1  }),
      capture:   m({ color: 0xd4a017, emissive: 0xff4455, emissiveInt: 0.55, transparent: true, opacity: 0.72, roughness: 0.3, metalness: 0.1  }),
      lastMov:   m({ color: 0x9a8840, emissive: 0x8877cc, emissiveInt: 0.30, transparent: true, opacity: 0.48, roughness: 0.4, metalness: 0.1  }),
      check:     m({ color: 0xff3344, emissive: 0xff3344, emissiveInt: 0.80, transparent: true, opacity: 0.90, roughness: 0.3, metalness: 0.1  }),
      pulse:     m({ color: 0xffd060, emissive: 0xffd060, emissiveInt: 1.0,  transparent: true, opacity: 0.80, roughness: 0.1, metalness: 0.1  }),
    };

    if (this.overhead) {
      // Ivory/cream marble — spec: polished, subtle veins, glossy reflections
      ...
      if (this.envMap) { wp.envMap = this.envMap; wp.envMapIntensity = 0.75; }
      this.M['whitePc'] = wp;

      // Glossy black onyx — spec: reflective polished surface, premium luxury
      ...
      if (this.envMap) { bp.envMap = this.envMap; bp.envMapIntensity = 0.80; }
      this.M['blackPc'] = bp;

      // Gold #D4AF37 — metallic, warm, base rings + king cross + queen crown
      const gp = new THREE.MeshPhysicalMaterial({
        color: 0xd4af37,
        roughness: 0.12,
        metalness: 1.0,
        emissive: new THREE.Color(0x8a6000),
        emissiveIntensity: 0.12,
      });
      if (this.envMap) { gp.envMap = this.envMap; gp.envMapIntensity = 1.4; }
      this.M['goldRing'] = gp;
    } else {
  ...
  private buildSharedGeos() {
    ...
    this.hoverRingGeo = new THREE.TorusGeometry(0.44, 0.038, 8, 36);

    // Hover ring mesh — persistent, toggled visible
    const hMat = new THREE.MeshStandardMaterial({
      color: 0xf0c840, emissive: new THREE.Color(0xf0c840), emissiveIntensity: 1.2,
      transparent: true, opacity: 0.85, roughness: 0.15, metalness: 0.3,
    });
    this.hoverRingMesh = new THREE.Mesh(this.hoverRingGeo, hMat);
    this.hoverRingMesh.rotation.x = -Math.PI / 2;
    this.hoverRingMesh.visible = false;
    this.scene.add(this.hoverRingMesh);
  }
  ...
  private startOverhead() {
    ...
    this.camera.lookAt(0, 0, 0);
    // Disabled for user — fixed cinematic angle per spec
    this.controls.enabled = false;
  ...
  private onIntroComplete() {
    ...
    // Snap to portrait-friendly view if needed
    const W = this.wrapperRef.nativeElement.clientWidth  || window.innerWidth;
    const H = this.wrapperRef.nativeElement.clientHeight || window.innerHeight;
    if (H > W) {
      this.camera.fov = 58;
  ...
  private spawnPieceAt(sq: Square, type: string, color: string): THREE.Group {
    ...
    const accentMat = this.M['goldRing'] ? this.M['goldRing'].clone() : mat;
    const g         = this.makePiece(type, mat, accentMat);
  ...
  private makePiece(type: string, mat: THREE.Material, accentMat?: THREE.Material): THREE.Group {
    const g = new THREE.Group();
    const add = (geo: THREE.BufferGeometry, dy = 0, useMat?: THREE.Material) => {
      const m = new THREE.Mesh(geo, useMat ?? mat);
      m.position.y = dy; m.castShadow = true; m.receiveShadow = true; g.add(m);
    };
    const gold = accentMat ?? mat;
    ...
    switch (type) {
      ...
      case 'q': {
        ...
        // Gold crown torus
        add(new THREE.TorusGeometry(0.155, 0.038, 10, 24), 0.68, gold);
        // 8 gold crown ball points
        for (let i = 0; i < 8; i++) {
          const a = (i / 8) * Math.PI * 2;
          const ball = new THREE.Mesh(new THREE.SphereGeometry(0.040, 10, 8), gold);
      ...
      case 'k': {
        ...
        // Gold crown torus
        add(new THREE.TorusGeometry(0.14, 0.034, 10, 24), 0.72, gold);
        // Gold cross
        add(new THREE.BoxGeometry(0.28, 0.068, 0.068), 0.815, gold);
        add(new THREE.BoxGeometry(0.068, 0.24,  0.068), 0.87,  gold);
        break;
      }
    }
    return g;
  ...
  private refreshHighlights() {
    ...
    if (this.selectedSquare) {
      addPlane(this.selectedSquare, this.M['selected']);
      addRing(this.selectedSquare, m({ color: 0xf7c230, emissive: 0xf7c230, emissiveInt: 1.0, transparent: true, opacity: 0.92, roughness: 0.12, metalness: 0.2 }));
    }
  ...
  private animateHighlights(now: number) {
    // Gentle pulse on highlights
    for (const [key, obj] of this.hlMap) {
      const mesh = obj as THREE.Mesh;
      const mat  = mesh.material as THREE.MeshStandardMaterial;
      if (key.endsWith('_p') && mat.emissiveIntensity !== undefined) {
        mat.emissiveIntensity = 0.45 + Math.sin(now * 0.0035) * 0.25;
      }
      if (key.endsWith('_r') && mat.emissiveIntensity !== undefined) {
        mat.emissiveIntensity = 0.80 + Math.sin(now * 0.005) * 0.35;
      }
    }

    // Hover ring pulse
    if (this.hoverRingMesh?.visible) {
      const mat = this.hoverRingMesh.material as THREE.MeshStandardMaterial;
      mat.emissiveIntensity = 1.0 + Math.sin(now * 0.007) * 0.5;
      mat.opacity = 0.80 + Math.sin(now * 0.007) * 0.12;
      mat.needsUpdate = true;
    }

    // Hover piece lift / non-hover settle
    for (const [sq, g] of this.pieceMap) {
      if (this.pieceAnims.some(a => a.group === g)) continue;
      const base   = this.sqToVec3(sq as Square);
      const target = base.y + this.yOff(g.userData['type']) + (sq === this.hoveredSq ? 0.30 : 0);
      g.position.y += (target - g.position.y) * 0.14;
    }
  }
    ...
    this.resizeObs = new ResizeObserver(entries => {
      for (const e of entries) {
        ...
        if (width > 0 && height > 0) {
          ...
          this.camera.updateProjectionMatrix();
          // Adjust camera for portrait/landscape transition
          if (this.phase === 'play') {
            this.applyCameraForViewport(width, height);
          }
        }
      }
    });
  ...
  /** Choose camera position based on viewport aspect ratio */
  private applyCameraForViewport(W: number, H: number) {
    if (this.camFlip) return; // let flip animation finish
    const portrait = H > W;
    if (this.overhead) {
      // Portrait: more top-down so board fills width nicely
      if (portrait) {
        this.camera.fov = 50;
        this.camera.position.set(0, 12, 7);
      } else {
      ...
      // Cinematic non-overhead after intro — reposition for portrait
      if (portrait) {
        this.camera.fov = 58;
        this.camera.position.set(0, 11, 6);
        this.camera.lookAt(0, 0, 0);
  ...
  private setupEvents() {
    ...
    canvas.addEventListener('mousemove', (e: MouseEvent) => {
      const sq = this.squareFromMouse(e.clientX, e.clientY);
      this.hoveredSq = (sq && this.pieceMap.has(sq)) ? sq : null;
      if (this.hoverRingMesh) {
        if (this.hoveredSq) {
          const p = this.sqToVec3(this.hoveredSq);
          this.hoverRingMesh.position.set(p.x, 0.1, p.z);
          this.hoverRingMesh.visible = true;
        } else {
          this.hoverRingMesh.visible = false;
        }
      }
    ...
    canvas.addEventListener('mouseleave', () => {
      this.hoveredSq = null;
      if (this.hoverRingMesh) this.hoverRingMesh.visible = false;
    });
